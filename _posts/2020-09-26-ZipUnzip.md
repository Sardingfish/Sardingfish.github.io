...
Win10开始winwods支持linux子系统WSL(Windows Subsystem for Linux)，两者共享文件系统(/mnt/win盘符)，可以非常方便地在任意位置使用linux命令，故本文仅介绍Linux压缩命令。（有测站名为COM1和COM2，在Windows下这两个文件名的文件夹和文件无法删除/创建，这种情况下打开WSL，就可以愉快的操作了:-)）
